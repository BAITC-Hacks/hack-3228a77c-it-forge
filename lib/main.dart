...
const blue = Color(0xFF0037B0);
const purple = Color(0xFF712AE2);
const wash = Color(0xFFF3F4FF);
const ink = Color(0xFF131B2E);

void main() => runApp(const WorkAiApp());
...
class WorkAiApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: 'Work.ai',
      theme: ThemeData(useMaterial3: true, colorScheme: ColorScheme.fromSeed(seedColor: blue), scaffoldBackgroundColor: const Color(0xFFFAF8FF)),
      home: const AppShell(),
    );
  }
}
...
class _AppShellState extends State<AppShell> {
  ...
  Widget build(BuildContext context) {
    final screens = <Widget>[
      const FeedScreen(),
      const Center(child: Text('Жауаптар', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
      const Center(child: Text('Менің тапсырмаларым', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
      const Center(child: Text('Чаттар', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
      const AuthScreen(),
    ];
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          NavigationDestination(icon: Icon(Icons.search), label: 'Іздеу'),
          NavigationDestination(icon: Icon(Icons.mark_email_read_outlined), label: 'Жауаптар'),
          NavigationDestination(icon: Icon(Icons.layers_outlined), label: 'Тапсырмалар'),
